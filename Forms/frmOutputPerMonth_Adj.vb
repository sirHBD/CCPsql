...
Public Class frmOutputPerMonth_Adj
    ...
    Private Sub frmOutputPerMonth_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            MsgBox("Unable to load the data.  See below and contact the system administrator" & vbCrLf & ex.Message, , My.Settings.SysName)
        End Try
        ...
        For i = 0 To xMonths.Count - 1
            ...
            If IsDBNull(DsOHDB.lstTotalsperMonth.Compute("Sum(decGrossIncome)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'")) = False Then
                dgv.Rows(1).Cells(i + 1).Value = Round(DsOHDB.lstTotalsperMonth.Compute("Sum(decGrossIncome)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'"), 0)
            Else
        ...
        For i = 0 To xMonths.Count - 1
            ...
            Else
                ...
                dgv.Rows(2).Cells(i + 1).Value = Round(CDec(DsOHDB.lstMatCostperMonth.Compute("Sum(TotMatCost)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'")), 0)
            End If
        ...
        For i = 0 To xMonths.Count - 1
            ...
            Try
                If IsDBNull(DsOHDB.lstLabCostperMonth.Compute("Sum(TotAdjCost)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'")) Then
                    xLabUnitCost = 0
                Else
                    xLabUnitCost = CDec(DsOHDB.lstLabCostperMonth.Compute("Sum(TotAdjCost)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'"))
                    dgv.Rows(3).Cells(i + 1).Value = Round(CDec(DsOHDB.lstLabCostperMonth.Compute("Sum(TotAdjCost)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'")), 0)
                End If
            Catch ex As Exception
            End Try
        ...
        Dim xOH1 As String = MiscCode.GetOHRate
        Dim xOHRate As Double = CDec(xOH / xTotUnits)
        Dim xCheck2070, xCheck127, xCheck89 As Decimal

        For i = 0 To xMonths.Count - 1

            'Get product Mix
            Dim x2070, x127, x89 As Decimal
            Try
                x2070 = DsOHDB.lstTotalsperMonth.Compute("Sum(decVolume)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'" & " AND txtProdSpec1 = '2070'")
                x2070 = x2070 * 2.07
                xCheck2070 = xCheck2070 + x2070
                x127 = DsOHDB.lstTotalsperMonth.Compute("Sum(decVolume)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'" & " AND txtProdSpec1 = '127'")
                x127 = x127 * 0.127
                xCheck127 = xCheck127 + x127
                x89 = DsOHDB.lstTotalsperMonth.Compute("Sum(decVolume)", "txtPeriodDescriptor = " & "'" & xMonths(i) & "'" & " AND txtProdSpec1 = '89'")
                x89 = x89 * 0.089
                xCheck89 = xCheck89 + x89
            Catch ex As Exception
            End Try
            ...
            dgv.Rows(5).Cells(i + 1).Value = Round((xOH1 * (x2070 + x127 + x89)), 2)
    ...
    Private Sub PrintRecordToolStripMenuItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles PrintRecordToolStripMenuItem.Click
        ...
        For iC = 0 To 13 'dgv.Columns.Count - 1
        ...
        For iX = 1 To dgv.Rows.Count - 1
            For iY = 0 To 13 'dgv.Columns.Count - 1
